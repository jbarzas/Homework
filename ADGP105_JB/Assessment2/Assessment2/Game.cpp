...
int main()
{
	...
	// creates a instance of the Player class named player defined as the constructor function.
	Player player = Player();

	// creates a instance of the Gold class named gold defined as the constructor function.
	Gold gold = Gold();

	// creates a instance of the Wumpus class named wumpus defined as the constructor function.
	...
	if (winCondition == false)
	{
		...
		cout << "Use 'w','a','s','d' as the arrow keys to move, or 'f' to fire your arrow, or 'q' to save or 'e' to load a save." << endl;
		cout << "" << endl;
		...
		do
		{
			...
			if (!(input == 'w' || input == 'a' || input == 's' || input == 'd' || input == 'q' || input == 'e' || input == 'f'))
			{
			...
			case 'd':
				...
				// asks the player what direction to shoot the arrow in and moves the arrow one space.
				// sets the arrows location to that of the players then moves it one space in the chosen direction.
			case 'f':
				arrow.location.x = player.location.x, arrow.location.y = player.location.y;
				cout << "what direction do you shoot the arrow?" << endl;
				cin >> input;

				if (input == 'w')
				{
					arrow.location.x, arrow.location.y += 1;
				}
				if (input == 's')
				{
					arrow.location.x, arrow.location.y -= 1;
				...
				if (input == 'a')
				{
					arrow.location.x -= 1, arrow.location.y;
				}
				if (input == 'd')
				{
					arrow.location.x += 1, arrow.location.y;
			...
			// if the arrows "x" and "y" coordinates are equal to that of the Wumpus it sets the alive bool of the Wumpus to false.
			if ((arrow.location.x == wumpus.location.x) && (arrow.location.x == wumpus.location.x) && (wumpus.alive == true))
			{
				wumpus.alive = false;
